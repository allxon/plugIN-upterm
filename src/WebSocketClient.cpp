...
void SendNotifyPluginStates(CWebSocketClient *ptr /*, bitset<4> statesUpdated*/)
{
    ...
    //     char *statesNotifyString = plugin->SetNotifyStates("upterm", states);
...
void CWebSocketClient::LoadNpUpdateTemplateFile(const std::string &path)
{
    auto json_source_string = getJsonFromFile(path);
    // Allxon::JsonValidator json_validator("plugin_upterm", , PLUGIN_ACCESS_KEY, CMAKE_PROJECT_VERSION);

    // if(!json_validator.Sign(json_source_string)) {
    //     std::cout << json_validator.error_message() << std::endl;
    //     return false;
    // }

    // std::cout << "Sign pass" << std::endl;
    // std::cout << "Signed result:" << std::endl << json_source_string << std::endl;

    // if (!json_validator.Verify(json_source_string)) {
    //     std::cout << json_validator.error_message() << std::endl;
    //     return false;
    // }

    std::cout << "Verify pass" << std::endl << std::endl;
}